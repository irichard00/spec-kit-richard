...
generate_commands() {
  ...
  for template in templates/commands/*.md; do
    ...
    case $ext in
      toml)
        body=$(printf '%s\n' "$body" | sed 's/\\/\\\\/g')
        { echo "description = \"$description\""; echo; echo "prompt = \"\"\""; echo "$body"; echo "\"\"\""; } > "$output_dir/rr.$name.$ext" ;;
      md)
        echo "$body" > "$output_dir/rr.$name.$ext" ;;
      agent.md)
        echo "$body" > "$output_dir/rr.$name.$ext" ;;
    esac
  done
...
generate_copilot_prompts() {
  ...
  for agent_file in "$agents_dir"/rr.*.agent.md; do
