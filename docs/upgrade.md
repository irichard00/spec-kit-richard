...
Once you've run `specify init`, the slash commands (like `/rr.specify`, `/rr.plan`, etc.) are **permanently installed** in your project's agent folder (`.claude/`, `.github/prompts/`, etc.). Your AI assistant reads these command files directly—no need to run `specify` again.
...
- **Test new slash commands:** Run `/rr.constitution` or another command to verify everything works
