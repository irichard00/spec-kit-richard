...
- Updated /rr.implement_all to use stacked branching (Folder 2 branches from Folder 1), preserving code dependencies across batched implementations.
- Updated `start-implementation` scripts to support `--from-current-branch` flag.

## [0.0.23] - 2026-02-17
 
- Harmonized /rr.implement and /rr.implement_all commands to ensure robust phase completion and end-to-end workflow (commit, push, PR creation).
- Integrated mandatory unit test requirements into /rr.plan, /rr.tasks, /rr.implement, and /rr.implement_all commands.
...
- Use the `/rr.` command prefix to easily discover Spec Kit-related commands.
...
- All command files now prefixed with `rr.` (e.g., `rr.specify.md`, `rr.plan.md`) for better discoverability and differentiation in IDE/CLI command palettes and file explorers
